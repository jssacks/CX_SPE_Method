...
## remove Salicyclic acid and pyroglutamic acid as they have poor reproducibility and problems with blanks
...
fig.1 <- ggplot(dat.fig, aes(x = sample, y = mean.Nmol.C, fill = reorder(Compound, order))) +
 ...
 # scale_fill_manual(values = lacroix_palette(type = "paired", n = 12)) +
  scale_y_continuous(expand = c(0, NA, NA, 175), limits = c(0,175)) +
...
####Make additional supplemental figure with nM compound not nM C
###get 12 most abundant compounds
ma.dat <- dat %>%
  group_by(Compound) %>%
  filter(!Compound == "Salicylic Acid") %>%
...
fig.3 <- ggplot(dat.fig, aes(x = sample, y = mean.conc, fill = reorder(Compound, order))) +
  ...
  scale_fill_brewer(palette = "Paired")+
  # scale_fill_manual(values = lacroix_palette(type = "paired", n = 12)) +
  scale_y_continuous(expand = c(0, NA, NA, 175), limits = c(0,21)) +
  theme(legend.text=element_text(size=rel(0.45)),
        axis.text=element_text(size=rel(0.5)),
        #  legend.title = element_text(size=rel(0.7)),
        axis.title=element_text(size=rel(0.7))) +
...
ggsave(filename = "Figures/Outputs/Enviro_Conc_PPLSPE_nM_supp.pdf", height = 5, width = 5)
