...
samp.key.file <- "Meta_Data/Sample_Lists/CXSPE_ES_RP_SampList.csv"
is.dat.file.1 <- "Intermediates/Environmental_Samples/ES_CX_RP_IS.csv"
is.dat.file.2 <- NA
xcms.dat.pos.file <- "Raw_Data/MS_Dial_Output/RP/Area_0_2021818167.txt"
xcms.dat.neg.file <- NA
cut.off <- 0.2
cut.off2 <- 0.1
...
xcms.dat_pos <- read_delim(xcms.dat.pos.file,
  ...
  select(MF, `210329_Blk_Blank1_A`:`210329_Smp_PS_C`) %>%
  pivot_longer(-MF, names_to = "SampID", values_to = "Area")

xcms.dat <- xcms.dat_pos %>%
...
is.dat.full.with.samp.edited <- is.dat.full.with.samp %>%
  filter(str_detect(MF, "Indole") |
           str_detect(MF, "Phenyl") |
...
BMIS_normalizedData.2 <- BMIS_normalizedData %>%
  mutate(Adjusted_Area = case_when(FinalBMIS == "Inj_vol" ~ 15*Adjusted_Area,
                                   !FinalBMIS == "Inj_vol" ~ Adjusted_Area)) 


BMISlist <- list(IS_inspectPlot, QuickReport, ISTest_plot, BMIS_normalizedData.2)
write_csv(BMIS_normalizedData.2, file = "Intermediates/Untargeted/Untargeted_RP_BMIS_Dat.csv")
